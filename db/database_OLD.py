...
async def save_condition(
    ...
    """,
        ...
        extraction.get('needs_review', False),
        extraction.get('applies_to_children', True),
        extraction.get('applies_to_adults', True),
        extraction.get('applies_to_pregnant'),
    )
...
async def get_conditions_for_symptoms(
    ...
    rows = await conn.fetch("""
        ...
        FROM conditions c
        JOIN clinical_relationships cr ON cr.target_entity_id = (
            SELECT id FROM clinical_entities WHERE canonical_name = c.name LIMIT 1
        )
